...
class ExpectedSARSAAgent:
    ...
    def __init__ (
        self,
        n_states: int, # tổng số state của môi trường 
        n_actions: int, # tổng số action
        alpha: float=0.1, #learningrate, tốc độ học
        gamma: float=0.95, #discount factor, mức độ quan tâm reward tương lai
        epsilon_start: float = 1.0, #epsilon ban đầu
        epsilon_end: float = 0.01, #epsilon nhỏ nhất
        epsilon_decay: float = 0.995, # Tốc độ giảm của epsilon
        seed: int = None, # seed để random có thể tái lập
    ...
    def select_action (self, state : int , eval_mode = False) -> int : 
        """
